...
| S-5 | Account takeover via credential compromise (no second factor) | Cognito | Medium | TOTP MFA is **required** (`Mfa.REQUIRED`, SMS/email factors disabled); the frontend's Amplify `<Authenticator>` natively drives the `CONTINUE_SIGN_IN_WITH_TOTP_SETUP` challenge, so admin-provisioned users complete QR-code TOTP enrollment at first sign-in | Mitigated |
...
| T-4 | Prompt injection via agent input (uploaded file content, signal name/description, chat message) | Bedrock Agent Core, signal_processor.py, agent_core.py, s3_reader.py | High | Bedrock Guardrail with a PROMPT_ATTACK content filter applied to every model invocation — enforced **fail-closed**: the agent refuses to start when `guardrail_id`/`guardrail_version` are unset (unless `ALLOW_UNGUARDED_AGENT=true` is explicitly exported for local development), and `deploy_agent.sh` auto-syncs the stack's guardrail outputs into `config.yaml`; unit tests pin the fail-closed wiring and untrusted-content tagging, and an ApplyGuardrail integration test (`GUARDRAIL_INTEGRATION=1`) proves a known prompt-attack payload inside `<untrusted_file_content>` is blocked by the deployed guardrail; all untrusted input (signal metadata, S3 trigger payload, file content) is wrapped in explicit `<untrusted_*>` tags with an accompanying system-prompt instruction to treat tagged content as data, never as instructions; bounded `max_iterations` (10, i.e. `recursion_limit=20`) and loop detection; a per-session `CircuitBreaker` caps identical back-to-back queries | Mitigated (defense-in-depth; residual risk accepted, see AR-3) |
| T-5 | Malicious S3 file triggers unintended *unreviewed* agent behavior via `autoExecute` | Signal processor, signal_management.py | High | `autoExecute` can only be set on a signal whose `bucketName` is the single platform-managed signal-uploads bucket (enforced server-side, not just IAM); auto-fired jobs are rate-limited per signal (`AUTO_EXECUTE_COOLDOWN_SECONDS`, default 60s) via an **atomic conditional write** on the signals table (`lastAutoExecuteAt` is claimed with a DynamoDB `ConditionExpression`, so a concurrent burst of uploads cannot all pass a stale read-then-act check — at most one firing per signal per window wins the claim; the rest are created `idle` for manual review) | Mitigated |
...
| ID  | Threat                                   | Component                  | Risk   | Mitigation                                                                                          | Status                                                     |
| --- | ------------------------------------------- | ------------------------------- | ------ | ------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------- |
| D-1 | API request flooding                     | API Gateway                | Medium | **Stage-level** throttling (50 rps / 100 burst) on the `prod` stage applies to every request regardless of API key (usage-plan throttles alone bind only API-key traffic, and this API issues no keys); a usage plan adds a 100k/day quota ceiling; optional AWS WAFv2 web ACL (managed rule groups + per-IP rate limiting + optional IP allowlist) gated behind `enable_waf: true`, wired from `config.yml` into the stack | Mitigated |
...
| D-5 | Large conversation payloads              | Conversation store, chat_management.py | Low    | Pagination (default limit 50); DynamoDB 400KB item limit provides natural bound; chat messages are capped at 8000 characters before being accepted, and the same 8000-character cap is enforced on `POST /conversations/{sessionId}` (conversation_management.py), which writes to the same conversation-store table the agent replays into model history — so the bound cannot be bypassed by posting the turn through the conversation endpoint instead of the chat endpoint | Mitigated |
...
| job_execution/chat_execution → AgentCore (cross-region) | `bedrock-agentcore:InvokeAgentRuntime` on the job-execution and chat-execution Lambda roles | **Mitigated** (AR-8 resolved). The former all-regions wildcard (`arn:aws:bedrock-agentcore:*:...`) has been replaced by an explicit region allowlist: the stack's own deploy region plus any region listed under `allowed_agent_regions` in `config.yml`. Registering an agent in another region now requires the operator to opt that region in; no other region is invocable. ARN validation at registration time (T-7) still applies on top. |
...
| AR-6 | Signal processor cross-tenant triggering on a shared bucket | This is a deliberate design property being accepted, stated explicitly: **all users of a deployment share the single platform-managed uploads bucket, and signal matching is bucket name plus user-chosen free-text prefix/suffix filters — nothing in the match ties an upload to the uploader**. Signals are matched without per-user scoping in the processor's match filter, so any user's upload can trigger any other user's signal whose prefix/suffix filters happen to match (including with `autoExecute`, where the agent then runs over the other user's file unreviewed). Any two users who each create an `s3_file_upload` signal with no prefix filter (or with overlapping prefixes) will both be triggered by the same upload, including an `autoExecute` firing. The per-signal auto-execute cooldown (T-5, enforced via an atomic conditional write) bounds how fast cross-user triggering can burn Bedrock invocations, but does not prevent it. This is accepted for a sample/educational project on the basis that: (a) all users of a given deployment are admin-provisioned (no self-signup) and implicitly a trusted cohort, and (b) the UI's bucket field is fixed and read-only, but the prefix field is still a free-text field the user controls, so a deployment operator can mitigate this today by having each user adopt a disjoint prefix convention (e.g. `users/{email}/`) — the platform does not currently enforce or assign this. Production deployments should not accept this risk without adding real per-user prefix scoping or a fully bucket-per-user model. | Platform team |
| AR-7 | ~~Cognito MFA is optional (`Mfa.OPTIONAL`), not required~~ **RESOLVED** | No longer an accepted risk: the user pool now sets `Mfa.REQUIRED` (TOTP-only). The original justification — no first-login enrollment UI — turned out not to hold: the frontend's Amplify `<Authenticator>` handles the TOTP setup and code-entry challenge steps natively, so admin-provisioned users are walked through enrollment at their first sign-in with no custom UI. SMS MFA remains deliberately not offered. See S-5. | Platform team |
| AR-8 | ~~`bedrock-agentcore:InvokeAgentRuntime` granted with a region wildcard~~ **RESOLVED** | No longer an accepted risk: both Lambda roles are now scoped to an explicit region allowlist (the stack's own region plus `allowed_agent_regions` from `config.yml`) instead of `arn:aws:bedrock-agentcore:*:...`. Cross-region agents remain supported, but each additional region is an explicit operator opt-in rather than an implicit grant on every AWS region. | Platform team |
...
| Medium   | Require MFA (not just offer it)                                                                         | S-5, AR-7        | **Done** — `Mfa.REQUIRED` (TOTP); Amplify `<Authenticator>` handles first-login enrollment |
